...
British Columbia has a network of air monitoring stations that
measure ground-level ozone and other air contaminants. This indicator reports on the concentration of ground-level ozone from two reporting periods, `r year_range_previous` and `r year_range`. The data are reported as a metric based on three consecutive years of data. These metrics are compared against the 
[Canadian Ambient Air Quality Standards](https://www.ccme.ca/en/air-quality-report#slide-7) to determine achievement and to define colour-based management levels. CAAQS were established by the 
[Canadian Council of Ministers of the Environment](http://www.ccme.ca) in 2012 as a key element of the Air Quality Management System that collaboratively and comprehensively manages air quality in Canada.

-   **Ground-level ozone is a type of air contaminant.** While ozone high up in the
    Earth's atmosphere occurs naturally and helps protect us from the sun's ultraviolet rays, ozone at 
    ground level is a harmful pollutant. It forms when pollution from vehicles, industry, and other sources reacts in sunlight.
...
-   **2021-2023: ozone levels met the Canadian Ambient Air Quality Standard at 
    `r summary_prev$n_achieved` of the `r summary_prev$n_stations` 
...
-   **2022-2024: ozone levels met the Canadian Ambient Air Quality Standard at 
    `r print_summary$n_achieved` of the `r print_summary$n` 
    B.C. ozone monitoring stations.** 
    ...
    There were `r if (print_summary$n_not_achieved == 0) {"no"} else {to_titlecase(as.character(english(print_summary$n_not_achieved)))}`
    stations reporting ozone levels \>`r ozone_lvls` ppb.
...
\fbox{
  \begin{minipage}[c]{15cm}
    ...
    Ozone levels are calculated using a statistical form called the 8-hour metric.
    
    The Canadian Ambient Air Quality Standard value for ozone applied in the 2021-2023 and 2022-2024 reporting periods is 
          \textbf{`r ozone_lvls` parts per billion (ppb)}. 
    
    \vspace{2mm}
    
    The ozone standard is achieved when ozone metrics are $\leq$`r ozone_lvls` ppb. The next reporting period (2023-2025) will apply the more stringent 2025 CAAQS for ground-level ozone of 60 ppb.
...
where sufficient data were available for the `r year_range` and `r year_range_previous` reporting
periods.
...
-   The ozone metric for an air zone is the highest ozone metric reported from 
    monitoring stations within the air zone. 
-   Criteria for data completeness and sufficiency of the ozone metric are described in the [Guidance Document on Achievement Determination: Canadian Ambient Air Quality Standards for Ozone (2021)](https://ccme.ca/en/res/gdadforozonecaaqsen.pdf).
-   Air quality monitoring stations with at least two years of valid data from `r year_range` and `r year_range_previous` reporting periods were included in this indicator. Visit 
    [Current Air Quality Data](http://www2.gov.bc.ca/gov/content?id=292EC5BD62A149A192B7DD2CE155E33E) 
    for a complete list of currently active air quality monitoring stations across B.C.
...
-   [BC Lung Asssociation's BC State of the Air Reports](https://bclung.ca/lung-health/air-quality/state-of-the-air-report-archives/)
-   [Canadian Ambient Air Quality Standards](https://www.ccme.ca/en/air-quality-report#slide-7)
...
-   [Guidance Document on Achievement Determination: Canadian Ambient Air Quality Standards for Ozone (2021)](https://ccme.ca/en/res/gdadforozonecaaqsen.pdf)
...
-   Read individual [Air Zone reports](http://www2.gov.bc.ca/gov/content?id=C62C0F15A6E3494399B86437D705A866) on the 
achievement of the Canadian Ambient Air Quality Standards in B.C.
